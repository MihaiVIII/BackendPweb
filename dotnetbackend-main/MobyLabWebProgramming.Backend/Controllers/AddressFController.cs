...
public class AddressFController : AuthorizedController // Here we use the AuthorizedController as the base class because it derives ControllerBase and also has useful methods to retrieve user information.
{
    /// <summary>
    /// Inject the required services through the constructor.
    /// </summary>
    /// 
    protected readonly IAddressFService _addressFService;
    public AddressFController(IUserService userService, IAddressFService addressFService) : base(userService) // Also, you may pass constructor parameters to a base class constructor and call as specific constructor from the base class.
    {
        _addressFService = addressFService;
    }
    ...
    public async Task<ActionResult<RequestResponse<AddressDTO>>> GetById([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        var currentUser = await GetCurrentUser();

        return currentUser.Result != null ? 
            this.FromServiceResponse(await _addressFService.GetAddress(id)) : 
                                                                                                                                         ...
                                                                                                                                         // the PaginationSearchQueryParams properties to the object in the method parameter.
    {
        ...
        return currentUser.Result != null ?
            this.FromServiceResponse(await _addressFService.GetAddreses(pagination)) :
            this.ErrorMessageResult<PagedResponse<AddressDTO>>(currentUser.Error);
    }
    ...
    public async Task<ActionResult<RequestResponse>> Add([FromBody] AddressAddDTO address)
    {
        ...
        return currentUser.Result != null ?
            this.FromServiceResponse(await _addressFService.AddAddr(address, currentUser.Result)) :
    ...
    public async Task<ActionResult<RequestResponse>> Update([FromBody] AddressUpdateDTO address) // The FromBody attribute indicates that the parameter is deserialized from the JSON body.
    {
        var currentUser = await GetCurrentUser();

        return currentUser.Result != null ?
            this.FromServiceResponse(await _addressFService.UpdateAddr(address, currentUser.Result)) :
            this.ErrorMessageResult(currentUser.Error);
    }
    ...
    public async Task<ActionResult<RequestResponse>> Delete([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        ...
        return currentUser.Result != null ?
            this.FromServiceResponse(await _addressFService.DeleteAddr(id, currentUser.Result)) :
            this.ErrorMessageResult(currentUser.Error);
    }
}
